...
function header() {
  doc.rect(0, 0, doc.page.width, 122).fill(COLORS.brand)
  doc
    .fillColor('#ffffff')
    .font('Helvetica-Bold')
    .fontSize(22)
  ...
  doc.text('linkedin.com/in/julianfuentesclavijo  |  github.com/jufe941209', 48, 92)
  doc.text('portafoliojulianfuentesclavijo.vercel.app  |  new-life-iota.vercel.app', 48, 106)
  doc.y = 142
...
function highlightBox(text) {
  ...
  const textHeight = doc.heightOfString(text, { width: textWidth, align: 'justify' })
  const boxHeight = textHeight + boxPadding * 2
  const boxY = doc.y
  doc.roundedRect(48, boxY, pageWidth, boxHeight, 4).fill('#EEF2FF')
  doc.rect(48, boxY, 4, boxHeight).fill(COLORS.accent)
  doc
    .fillColor(COLORS.brand)
    .font('Helvetica-Bold')
    .fontSize(9.5)
    .text(text, 48 + boxPadding, boxY + boxPadding, { width: textWidth, align: 'justify' })
...
paragraph(
  'Diferenciador clave: transición deliberada del entrenamiento deportivo de élite al desarrollo de software — demostrando capacidad de aprendizaje excepcional, resiliencia bajo presión y mentalidad de producto centrada en el usuario. Disponible para trabajo remoto, async-first, con equipos internacionales.',
)
doc.moveDown(0.2)
highlightBox(
  'Obtuve uno de los mejores resultados de Colombia en las pruebas Saber TyT (ICFES) 2026, con puntaje global de 141/200: percentil 100 a nivel nacional (superé al total de los estudiantes del país que presentaron la prueba) y percentil 99 en el núcleo de Ingeniería de Sistemas y Telemática (superé a 99 de cada 100 estudiantes de programas afines a mi campo de formación). Nivel máximo de desempeño en Lectura Crítica, Comunicación Escrita, Razonamiento Cuantitativo y Competencias Ciudadanas; Inglés B2.',
)
...
skillLine(
  ...
  'Jasmine/Karma (Angular), Diagramas de Flujo de Procesos (Sistemas ADK), Manuales Técnicos, Manuales de Usuario, Manuales de Caza de Fallas, Actas de Entrega',
)
...
bullet('Integré APIs RESTful en múltiples microservicios institucionales; produje documentación técnica, manuales de usuario, manuales de caza de fallas y diagramas de procesos ADK para más de 5 plataformas web (60+ módulos).')
bullet('Participé en ciclos de desarrollo ágil con Plane y Scrum — sprint planning y Git en equipos multidisciplinarios.')
...
bullet('NewLife E-Commerce: plataforma fullstack completa — React, .NET 10/C# API RESTful con JWT, BCrypt, CORS, recuperación de contraseña por email (SMTP) e integración de pasarela de pago Wompi. SQL Server (11 tablas, 56+ stored procedures). CI/CD con GitHub Actions hacia Azure App Service.')
...
bullet('Desarrollé interfaces Angular + TypeScript para e-commerce y gestión de tareas en un equipo internacional remoto async-first.')
bullet('Lideré un equipo ágil de 5 desarrolladores — sprint planning, Jira, revisión de pull requests en Git y entrega puntual de todos los hitos.')
...
jobHeader(
  ...
  'CAFAM | Ride Now | Club Real Leones | Smart Fit | UNP | Fútbol Libre Fénix',
  'Bogotá, Colombia',
...
bullet('Entrené a más de 300 atletas en ciclismo, natación, triatlón, atletismo y fútbol — planes periodizados, pruebas fisiológicas (FTP, VO2max, evaluaciones antropométricas) y adaptación basada en datos.')
bullet('Preparé triatletas de élite para Ironman Cozumel (México) y el Triatlón de Vichy (Francia), como encargado de la preparación física, ciclística y de natación.')
...
sectionTitle('Certificaciones y Resultados Académicos')
bullet('Saber TyT (ICFES), 2026 — Puntaje global 141/200. Percentil 100 a nivel nacional; percentil 99 en el núcleo de Ingeniería de Sistemas y Telemática.')
